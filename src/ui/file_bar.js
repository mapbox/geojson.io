...
/*
var share from"./share"
  modal from"./modal.js"
  flash from"./flash"
  zoomextent from"../lib/zoomextent"
  readFile from"../lib/readfile"
  meta from"../lib/meta.js"
  saver from"../ui/saver.js" */
...
export default () => {
  ...
  var actions = [
    ...
    },
    {
      title: "New",
      action: function() {
        window.open(window.location.origin + window.location.pathname + "#new");
      }
    },
    {
      title: "Meta",
      ...
      {actions.map((item, i) => {
        return (
          <button
            key={i}
            style={{ zIndex: 999 }}
            className="bn pv1 ph2 br2 br--top outline-0 hide-child relative pointer black-50 hover-black"
          >
            {item.title}
            {item.children ? (
              <div
