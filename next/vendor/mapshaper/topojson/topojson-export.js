...
export function exportTopoJSON(dataset, opts) {
  var extension = '.' + (opts.extension || 'json'),
    needCopy =
      !opts.final ||
      (datasetHasPaths(dataset) && dataset.arcs.getRetainedInterval() > 0),
  ...
  if (opts.prettify) {
    stringify = getFormattedStringify(
      'coordinates,arcs,bbox,translate,scale'.split(',')
    );
  }
...
TopoJSON.calcExportBounds = function (bounds, arcs, opts) {
  ...
  if (opts.topojson_precision > 0) {
    ...
    unitXY = [
      bounds.width() / (opts.quantization - 1),
      bounds.height() / (opts.quantization - 1)
    ];
