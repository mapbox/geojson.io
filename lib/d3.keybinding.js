...
function keybinding(namespace) {
    ...
    keybinding.on = function(code, callback, capture) {
        ...
        for (var i = 0; i < code.length; i++) {
            ...
            if (code[i] in modifierCodes) {
                binding.event[modifierProperties[modifierCodes[code[i]]]] = true;
            } else if (code[i] in keyCodes) {
                binding.event.keyCode = keyCodes[code[i]];
            }
        }
